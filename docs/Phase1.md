...
- **Shell**: zsh 5.9
- **仓库**: `git@github.com:luzhenyuat-stack/Auto-WorkStation.git`
...
cd ~/Auto-WorkStation
git init
git remote add origin git@github.com:luzhenyuat-stack/Auto-WorkStation.git
...
bash
ssh-keygen -t ed25519 -C "luzhenyuat-stack"
cat ~/.ssh/id_ed25519.pub  # 复制公钥
# GitHub → Settings → SSH and GPG keys → New SSH key → 粘贴
验证后推送：

bash
ssh -T git@github.com  # 应显示：Hi luzhenyuat-stack! You've successfully authenticated...
git add .
...
chezmoi init --apply git@github.com:luzhenyuat-stack/Auto-WorkStation.git
...
当前状态快照（2026-06-17）
项目	状态
Git 仓库	git@github.com:luzhenyuat-stack/Auto-WorkStation.git
...
完成标志：执行 copier copy gh:luzhenyuat-stack/ai-project-template myapp 一条命令即可生成包含完整运行时配置的新项目。
